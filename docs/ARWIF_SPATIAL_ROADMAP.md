...
- initial support can now preserve validated room-aware context through top-level `room` metadata covering room dimensions, geometry reference, surface profile, surface treatment, reflection policy, renderer adaptation hints, listening zones, and initial speaker placement with compact speaker role and coverage intent
