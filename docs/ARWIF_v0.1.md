...
rwif arwif-batch-diff --left dist/alpha.baseline.arwif dist/beta.baseline.arwif --right dist/alpha.candidate.arwif dist/beta.candidate.arwif --json
...
For collection-scale comparison, the reference CLI also supports diffing multiple explicit left and right artifact pairs in one command while keeping the same state-level and metadata diff semantics as single-artifact `arwif-diff`.
