...
- `ROOM_REVIEW_baseline_v0_1.yaml`: baseline Level 3 room-aware review spec covering dimensions, geometry reference, surface treatment, reflection policy, renderer adaptation hints, listening zones, speaker placement, and speaker coverage intent
...
That review payload is intentionally non-trivial: it should surface room dimension, geometry-reference, surface-profile, surface-treatment, reflection-policy, renderer-adaptation, listening-zone, speaker-placement, and speaker-coverage drift in one compact batch-review document.
