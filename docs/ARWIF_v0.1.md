...
- an initial Level 3 room-aware slice via top-level `room` metadata for room dimensions, surface profile, reflection policy, renderer adaptation hints, listening zones, and speaker placement
...
- `room` as a mapping that may contain:
  ...
  - `speakers` as a list of `{ speaker_id, anchor, channel?, role? }` mappings, where `channel` must match the declared `channel_layout` when present
...
- `room` for structured room-aware context including dimensions, surface profile, reflection policy, renderer adaptation hints, listening zones, and speaker placement
- `sample_rate_hz`
