...
- an initial Level 3 room-aware slice via top-level `room` metadata for room dimensions, surface profile, reflection policy, listening zones, and speaker placement
...
- `room` for structured room-aware context including dimensions, surface profile, reflection policy, listening zones, and speaker placement
- `sample_rate_hz`
